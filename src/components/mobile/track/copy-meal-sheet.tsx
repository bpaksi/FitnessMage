...
interface CopyMealSheetProps {
  ...
  open: boolean
  onClose: () => void
  onCopy: (sourceDate: string, sourceMealType: MealType) => void
}

export function CopyMealSheet({ targetDate, targetMealType, open, onClose, onCopy }: CopyMealSheetProps) {
    ...
    <Drawer open={open} onOpenChange={(o) => !o && onClose()}>
      <DrawerContent className="border-[#1e293b] bg-[#0f172a]">
        ...
        <div className="space-y-4 px-4 pb-8">
          ...
          <Button
            onClick={() => onCopy(sourceDate, sourceMealType)}
